...
RequestResponse handleResponse(Response? response, {RequestTransformer? requestTransformer}) {
  requestTransformer ??= DefaultRequestTransformer.getInstance();

  // 返回值异常
  if (response == null) {
    return RequestResponse.failureFromError();
  }

  // token失效
  if (_isTokenTimeout(response.statusCode)) {
    return RequestResponse.failureFromError(UnauthorisedException(message: "没有权限", code: response.statusCode));
  }

  // 接口调用成功
  if (_isRequestSuccess(response.statusCode)) {
    return requestTransformer.parse(response);
  } else {
    // 接口调用失败
    return RequestResponse.failure(errorMsg: response.statusMessage, errorCode: response.statusCode);
  }
}

/// 鉴权失败
bool _isTokenTimeout(int? code) => code == 401;
...
/// 处理错误
RequestResponse handleException(Exception exception) {
  final parseException = _parseException(exception);
  return RequestResponse.failureFromError(parseException);
}
